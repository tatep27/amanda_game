...
export class Ch2Scene extends Phaser.Scene {
  ...
  init(data: { spawnId?: string; previousScene?: string }) {
    ...
    console.log(`[Ch2Scene] init with spawn: ${this.spawnId}`);
  }

  create() {
    const manifest = this.cache.json.get('ch2') as SceneManifest;
    
    if (!manifest) {
      console.error('[Ch2Scene] Failed to load ch2.json');
      return;
    }
    ...
    if (manifest.introMessage && !Ch2Scene.visitedScenes.has('ch2')) {
      Ch2Scene.visitedScenes.add('ch2');
    ...
    if (isInteractPressed) {
      if (dialogueActive) {
        ...
        if (interacted instanceof Door) {
          ...
          console.log(`[Ch2Scene] Transitioning to ${toSceneKey} at spawn ${toSpawnId}`);
  ...
  private handlePlayerDeath(): void {
    ...
    console.log('[Ch2Scene] Player died! Restarting scene...');
