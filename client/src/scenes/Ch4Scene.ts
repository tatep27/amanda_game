...
export class Ch4Scene extends Phaser.Scene {
  ...
  init(data: { spawnId?: string; previousScene?: string }) {
    ...
    console.log(`[Ch4Scene] init with spawn: ${this.spawnId}`);
  }

  create() {
    const manifest = this.cache.json.get('ch4') as SceneManifest;
    
    if (!manifest) {
      console.error('[Ch4Scene] Failed to load ch4.json');
      return;
    }
    ...
    if (manifest.introMessage && !Ch4Scene.visitedScenes.has('ch4')) {
      Ch4Scene.visitedScenes.add('ch4');
      ...
      if (dialogueActive) {
        ...
        if (interacted) {
          if (interacted.constructor.name === 'Door') {
            ...
            console.log(`[Ch4Scene] Transitioning to ${toSceneKey} at spawn ${toSpawnId}`);
  ...
  private handlePlayerDeath(): void {
    ...
    console.log('[Ch4Scene] Player died! Restarting scene...');
